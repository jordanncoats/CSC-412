...
/** This function takes the filename vector and splits it in mostly even lots.
  ...
  */
std::vector<std::vector<std::string>> SplitVector(const std::vector<std::string> &fileName, const int k);
...
/** This function takes one of the split list of filenames and orders it by the process index.
  * to write to a text file of that process index.
...
int main(int argc, const char *argv[])
{
    ...
    //A loop to create distribute functions for each split list
    for (int i = 0; i < procNum; i++)
    {
        distributor(procNum, dataRootPath, fileName_Split[i]);
    }

    //A second loop to take the now organized distrubted lists and sort them by line number
    for (int i = 0; i < procNum; i++)
    {
        processor(i);
    }
    ...
    for (int i = 0; i < procNum; i++)
    {
        std::ifstream file("tempV1/output_" + std::to_string(i) + ".txt");
    ...
    nftw("tempV1", rmFiles, 10, FTW_DEPTH | FTW_MOUNT | FTW_PHYS);
    return 0;
}
...
//  process should handle the each.
//
//----------------------------------------------------------------------
void distributor(const int k, const std::string &path, std::vector<std::string> &fileName)
{
    ...
    for (int i = 0; i < k; i++)
    {
        ...
        std::ofstream out("tempV1/p_" + std::to_string(i) + ".txt", std::ios_base::app);
        out << output;
        out.close();
    }
}
...
//  to sort the contents by line number.
//
//----------------------------------------------------------------------
void processor(const int k)
{
    ...
    reader.open("tempV1/p_" + std::to_string(k) + ".txt");
    int i = 0;
    ...
    std::ofstream out("tempV1/output_" + std::to_string(k) + ".txt", std::ios_base::app);
    out << output;
    reader.close();
    out.close();
}

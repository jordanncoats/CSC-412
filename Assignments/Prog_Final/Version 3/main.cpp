 ...
 * 
 *  This is version 3 which introduces the grid mutex. This is to ensure the robots and boxes don't walk
 *  over each other.
 * 
 ***********************************************/
#include <iostream>
...
void initializeApplication(RobotScene *scene)
{
    ...
    int robotLoc[scene->numBoxes][2]; // = {{12, 5}, {5, 7}};
    int boxLoc[scene->numBoxes][2];   // = {{13, 5}, {5, 2}};
    int doorAssign[scene->numBoxes];  // = {0, 1};
    int doorLoc[scene->numDoors][2];  // = {{8, 5}, {4, 8}};
    ...
    */
    ...
    fclose(outputFile);
    /** create the grid mutex*/
    scene->grid_mutex = (pthread_mutex_t **)malloc(sizeof(pthread_mutex_t *) * scene->numRows);
    for (int i = 0; i < scene->numRows; i++)
    {
        scene->grid_mutex[i] = (pthread_mutex_t *)malloc(sizeof(pthread_mutex_t) * scene->numCols);
        for (int j = 0; j < scene->numCols; j++)
            pthread_mutex_init(&scene->grid_mutex[i][j], nullptr);
    }
